...
data=train_test_split(X,shuffle=False)
g=3
def cross_validate(hyperparameters=None):
    ...
    for X_train, X_test, y_train, y_test in dataset.get_splits(3):
        ...
        scores.append(pyteller.evaluate(test_data=all_output['actuals'],
...
scores = pyteller.evaluate(test_data=all_output['actuals'], forecast=all_output['forecast'],
                           metrics=['MAPE', 'sMAPE'])
